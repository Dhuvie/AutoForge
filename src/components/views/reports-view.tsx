...
export function ReportsView() {
  ...
  const reportMarkdown = useMemo(() => {
    ...
    return `# nexusml — Experiment Report
...
This report documents an automated machine learning experiment conducted on the **${dataset.filename ?? 'dataset'}** dataset using nexusml. The dataset contains **${dataset.rows.length.toLocaleString()} rows** across **${dataset.columns.length} columns**, with **${dataset.schema.filter((c) => c.role === 'feature').length} features** and target column **${dataset.profile.targetCandidate ?? '—'}**.
...
To run locally:
\`\`\`bash
docker run -p 8000:8000 -e nexusml_API_KEY=${deployment.apiKey} ${deployment.dockerImage}
\`\`\`
...
*Report generated by nexusml v1.0.0*
`;
